...
final class SymfonyConsoleRenderer implements ResultRenderer
{
    ...
    {
        $this->output->writeln('    <comment>PHPUnitObjective completed: ' . $result->points() . ' points.</comment>');
    }
    ...
    {
        $this->output->writeln('    <error>PHPUnitObjective failed with ' . $result->points() . '/' . $result->maxPoints() . ' points</error>');
    }
